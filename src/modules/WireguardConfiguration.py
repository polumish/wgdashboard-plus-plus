...
class WireguardConfiguration:
    ...
    def createDatabase(self, dbName = None):
        ...
        self.peersTable = sqlalchemy.Table(
            ...
            sqlalchemy.Column('opnsense_listen_port', sqlalchemy.Integer, server_default='0'),
    ...
    def __migrateAddIsGatewayColumn(self, dbName):
        """Add gateway-related columns to peer tables if missing (for DBs
        created before v1.02/v1.1). Uses dialect inspector to avoid blind
        ALTER errors."""
        try:
            ...
            additions = [
                ...
                ('opnsense_listen_port', 'INTEGER DEFAULT 0'),
            ]
            with self.engine.begin() as conn:
                for t in targets:
                    ...
                    for col, ddl in additions:
                        if col not in cols:
                            conn.execute(sqlalchemy.text(
                                f'ALTER TABLE "{t}" ADD COLUMN {col} {ddl}'))
        except Exception:
            pass
    ...
    def addPeers(self, peers: list) -> tuple[bool, list, str]:
        ...
        try:
            ...
            if self.getStatus():
                # Interface is running — apply peers live via wg set
                for p in peers:
                    ...
                    subprocess.check_output(
                        f"{self.Protocol} set {self.Name} peer {p['id']} "
                        f"allowed-ips {p['allowed_ip'].replace(' ', '')}"
                        f"{f' preshared-key {uid}' if presharedKeyExist else ''}",
                ...
                # Interface is DOWN — write [Peer] blocks directly into .conf
                # so wg-quick up will pick them up automatically
                with open(self.configPath, 'a') as f:
                    for p in peers:
                        f.write(f"\n[Peer]\nPublicKey = {p['id']}\n")
                        f.write(f"AllowedIPs = {p['allowed_ip'].replace(' ', '')}\n")
                        if p.get('preshared_key'):
                            f.write(f"PresharedKey = {p['preshared_key']}\n")
            self.getPeers()
            for p in peers:
